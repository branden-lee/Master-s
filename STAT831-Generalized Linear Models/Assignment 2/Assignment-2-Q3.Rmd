...
model3 <- glm(resp ~ Rainfall + I(Rainfall^2) + I(Rainfall^3), family = binomial, 
              data = toxo)
...
legend('topright', legend = c('Model 1', 'Model 2'), col=c('red', 'blue'), 
       pch=c(3,1), bty='n')
...
d)
```{r}
root1 <- uniroot(function(x) {model3$coefficients %*% c(1, x, x^2, x^3)},
                 lower=1600, upper=2300)$root
root2 <- uniroot(function(x) {model3$coefficients %*% c(1, x, x^2, x^3)},
                 lower=0, upper=1700)$root
root3 <- uniroot(function(x) {model3$coefficients %*% c(1, x, x^2, x^3)},
                 lower=root2, upper=root1)$root
```
...
$$0 = logit \left( \frac{1}{2} \right) = \hat{\beta}_0 + \hat{\beta}_1 x + \hat{\beta}_2 x^2 + \hat{\beta}_3 x^3$$
which are
$$\delta_{0.50, 1} = `r root1`,\ \delta_{0.50, 2} = `r root2`,\ \delta_{0.50, 3} = `r root3`$$
The maximum and minimum of the roots nearly cover the entire range of observed values of rainfall, and since they are both median effective doses, all values in between them are also median effective doses. That is to say, the data suggests that increased rainfall does not lead to increased number of positive toxoplasmosis tests.